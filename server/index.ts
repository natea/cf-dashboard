...
async function createApp() {
  ...
  // Connect event aggregator to storage for event flow (storage -> aggregator -> hub)
  aggregator.connectStorage(storage);
  ...
  app.onError((err, c) => {
    if (err instanceof HTTPException) {
      // Only log server errors, not client auth errors
      if (err.status >= 500) {
...
async function main() {
  ...
  console.log("");
  console.log("  Claims Dashboard API");
  console.log("  --------------------");
  console.log(`  env:  ${config.server.env}`);
  console.log(`  db:   ${config.database.type}`);
    ...
    websocket: {
      ...
      message(ws, message) {
        try {
          ...
          hub.handleError(ws, error as Error);
        }
      },
