...
export class Orchestrator {
  ...
  constructor(config: Partial<OrchestratorConfig> = {}, logger?: Logger) {
    ...
    this.agentSpawner = new AgentSpawner({
      ...
      useClaudeFlowCli: this.config.useClaudeFlow,
    });
  ...
  /**
   * Process a single claim: route it, claim it, then spawn an agent.
   *
   * Order is critical: claim BEFORE spawn to prevent zombie agents.
   * If claiming fails (409), no agent process is leaked.
   */
  private async processClaim(claim: Claim): Promise<void> {
    ...
    try {
      ...
      // Generate agent ID upfront so we can claim before spawning
      const agentId = `${routing.agentType}-${Math.random().toString(36).substring(2, 8)}`;

      // Step 1: Claim the issue FIRST (prevents zombie agents on 409)
      ...
      this.logger.info(`Claimed ${claim.id} for agent ${agentId}`);

      // Step 2: Spawn the agent with the pre-generated ID
      ...
      if (spawnResult.success && spawnResult.agentId) {
        ...
        // Spawn failed after claiming — release the claim so it goes back to backlog
        this.logger.warn(`Spawn failed after claiming, releasing ${claim.id}`);
        await this.dashboardClient.releaseClaim(claim.id).catch((e) => {
          this.logger.warn(
            `Failed to release claim ${claim.id}: ${e instanceof Error ? e.message : String(e)}`
          );
        });
  ...
  private handleWsMessage(msg: WsMessage): void {
    ...
    switch (msg.type) {
      ...
      case "pong":
        // Heartbeat responses — no action needed
        break;
