...
export class TaskRouter {
  ...
  private async routeViaCLI(
    ...
    return new Promise((resolve) => {
      ...
      try {
        // Use npx to run claude-flow CLI
        const proc = spawn(
          "npx",
          [
            "@claude-flow/cli",
        ...
        proc.on("close", (code) => {
          ...
          if (code !== 0) {
            this.logger.debug(`CLI exited with code ${code}: ${stderr}`);
  ...
  private routeViaHeuristics(context: TaskContext): RoutingResult {
    ...
    // Check for low complexity indicators (unless already upgraded to opus)
    if (modelTier !== "opus") {
      for (const pattern of COMPLEXITY_INDICATORS.low) {
        if (pattern.test(fullText)) {
          modelTier = "haiku";
          reasons.push(`Low complexity indicator: ${pattern}`);
          break;
        }
      }
    }
  ...
  async isClaudeFlowAvailable(): Promise<boolean> {
    ...
    return new Promise((resolve) => {
      const proc = spawn("npx", ["@claude-flow/cli", "--version"], {
        timeout: 5000,
        stdio: ["pipe", "pipe", "pipe"],
      });
...
/**
 * Create a task router with the console logger
 */
export function createTaskRouter(logger?: Logger): TaskRouter {
  return new TaskRouter(logger);
}
