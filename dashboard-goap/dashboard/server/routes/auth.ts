...
export function createAuthRoutes() {
  ...
  auth.post("/login", async (c) => {
    ...
    // Return user with token (token is the secret for simplicity)
    return c.json({
      id: `user-${Date.now()}`,
      name,
      token: secret,
  ...
  auth.get("/verify", (c) => {
    ...
    // Note: In a real app, you'd decode the token to get user info
    return c.json({
...
/**
 * Optional auth middleware - sets auth vars but doesn't reject
 */
